...
class AudioPickerFragment : BaseFragment() {
    ...
    private var mListener: AudioPickerFragmentListener? = null

    interface AudioPickerFragmentListener
    ...
    override fun onAttach(context: Context?) {
        ...
        if (context is AudioPickerFragmentListener) {
            mListener = context
        } else {
            throw RuntimeException(context?.toString() + " must implement AudioPickerFragmentListener")
        }
    }
        ...
        view.let {
            ...
            if (sectionsPagerAdapter != null) {
                for (index in 0 until sectionsPagerAdapter.count) {
                    val audioFragment = childFragmentManager
                            .findFragmentByTag(
                                    "android:switcher:" + R.id.viewPager + ":" + index) as AudioFragment
                    val fileType = audioFragment.fileType
                    if (fileType != null) {
                        val filesFilteredByType = filesMap[fileType]
                        if (filesFilteredByType != null)
                            audioFragment.updateList(filesFilteredByType)
                    }
                }
            }
        }
    ...
    private fun setUpViewPager() {
        ...
        for (index in supportedTypes.indices) {
            adapter.addFragment(AudioFragment.newInstance(supportedTypes[index]), supportedTypes[index].title)
        }
    ...
    companion object {
        ...
        fun newInstance(): AudioPickerFragment {
